...
class Decoder(nn.Module):
    def __init__(self, num_classes=20, in_channels=[32,64,160,256], embedding_dim=768, dropout_ratio=0.1):
        ...
        # 创建自适应上采样器
        self.upsample_c4 = AdaptiveUpsample(embedding_dim)
        self.upsample_c3 = AdaptiveUpsample(embedding_dim)
        self.upsample_c2 = AdaptiveUpsample(embedding_dim)
    ...
    def forward(self, inputs):
        ...
        # _c4 = F.interpolate(_c4, size=c1.size()[2:], mode='bicubic', align_corners=False)
        # _c4.clamp(min=0, max=255)

        _c3 = self.linear_c3(c3).permute(0, 2, 1).contiguous().reshape(n, -1, c3.shape[2], c3.shape[3])
        # _c3 = F.interpolate(_c3, size=c1.size()[2:], mode='bilinear', align_corners=False)
        # _c3 = F.interpolate(_c3, size=c1.size()[2:], mode='bicubic', align_corners=False)
        # _c3.clamp(min=0, max=255)

        _c2 = self.linear_c2(c2).permute(0, 2, 1).contiguous().reshape(n, -1, c2.shape[2], c2.shape[3])
        # _c2 = F.interpolate(_c2, size=c1.size()[2:], mode='bilinear', align_corners=False)
        # _c2 = F.interpolate(_c2, size=c1.size()[2:], mode='bicubic', align_corners=False)
        # _c2.clamp(min=0, max=255)

        _c1 = self.linear_c1(c1).permute(0, 2, 1).contiguous().reshape(n, -1, c1.shape[2], c1.shape[3])

        # 使用自适应上采样（核心修改部分）
        target_size = c1.shape[2:]  # 获取c1的HW尺寸
        _c4 = self.upsample_c4(_c4, target_size)
        _c3 = self.upsample_c3(_c3, target_size)
        _c2 = self.upsample_c2(_c2, target_size)
...
class SegFormer(nn.Module):
    def __init__(self, num_classes=21, phi='b0', pretrained=False, backbone_weight_path=''):
        ...
        self.decode_head = Decoder(num_classes, self.in_channels, self.embedding_dim)
